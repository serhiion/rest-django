...
class TestAircraftsModel(TestCase):
    ...
    def setUp(self):
        self.aircrafts_attributes = {'aircraft_code': '121',
                                     'model': 'Airbus 12',
                                     'range': 5000
                                     }
    ...
    def test__str__(self):
        aircraft_actual = str(Aircrafts.objects.get(aircraft_code='121'))
        aircraft_to_expect = "Code: 121 Model: 12 Range: 5000"
        self.assertEqual(aircraft_actual, aircraft_to_expect)

    def test_create(self):
        aircraft_actual = Aircrafts.objects.create(aircraft_code='122', model='14', range=5000)
        self.assertEqual(aircraft_actual.model, '14')

    def test_update(self):
        aircraft = Aircrafts.objects.get(aircraft_code='122')
        aircraft.aircraft_code = '123'
        aircraft.model = 'NewModel'
        aircraft.range = 5000
        self.assertIsInstance(aircraft, Aircrafts)
        self.assertEqual(aircraft.aircraft_code, '123')
        self.assertEqual(aircraft.model, 'NewModel')
        self.assertEqual(aircraft.range, 5000)
...
class AirportsTest(TestCase):
    ...
    def test_create(self):
        airport_actual = Airports.objects.create(airport_code='GG', airport_name='Test', city='Lviv',
                                                 longitude=36.2323, latitude=53.2312, timezone='Europe/Kyiv')
        self.assertEqual(airport_actual.city, 'Lviv')

    def test_update(self):
        airport = Airports.objects.get(city='Lviv')
        airport.airport_code = 'OAS'
        self.assertIsInstance(airport, Airports)
        self.assertEqual(airport.airport_code, 'OAS')
        self.assertEqual(airport.airport_name, 'Test')

    def test_delete(self):
        airport = Airports.objects.filter(city='Lviv')
        airport.delete()
